package homeworks.homework3;/* SList.java */
...
public class SList {
    ...
    public void squish() {
        ...
        int iterativeNumber = 0;
        SListNode sListNode = head;
        while (sListNode.next != null) {
            if (sListNode.next.item.equals(sListNode.item)) {
                iterativeNumber++;
                sListNode.next = sListNode.next.next;
            } else {
    ...
    public void twin() {
        ...
        if (head == null)
            return;
        int newHeadSize = 0;
        SListNode newHead = new SListNode(head.item, head);
        newHeadSize = 2;
        SListNode temp = null;
        temp = newHead.next;
        ...
        while (head.next != null) {
            head = head.next;
            temp.next = new SListNode(head.item, head);
            newHeadSize += 2;
            System.out.println("size : " + newHeadSize);
            temp = temp.next.next;
        }
        size = newHeadSize;
        head = newHead;
